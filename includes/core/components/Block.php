...
class Block_Component extends Component
{
	...
	public function render()
	{
		...
		//$this->c('Log')->writeDebug('%s : rendering template %s. Hash: %s', __METHOD__, $template, $this->m_uniqueHash);
		...
		$this->c('Page')->addContents($this->compile($template), $this->m_blockRegion);
	...
	{
		ob_start();
		foreach ($this->m_blockVariables as $_var => $_value)
			${$_var} = $_value;
