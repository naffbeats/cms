...
class Locale_Component extends Component
{
	...
	public function initialize()
	{
		...
		return $this->setLocale($locale, $this->GetLocaleIDForLocale($locale), true);
	}
	...
	private function getLocaleFile($type, $path, &$localeHolder)
	{
		...
		if (file_exists($path . 'locale_' . $this->m_localeName . '.php'))
		{
			...
			$localeHolder = $type == 'core' ? $Core_Locale : $Site_Locale;
		}
		elseif(file_exists($path . 'locale_' . $this->c('Config')->getValue('site.locale.default') . '.php'))
		{
			...
			$this->core->terminate(ucfirst($type) . ' locale was not found');
	...
	public function setLocale($locale_name, $locale_id, $load_locale = true)
	{
